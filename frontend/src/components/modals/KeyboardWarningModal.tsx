...
interface KeyboardWarningModalProps {
  isOpen: boolean;
  onClose: () => void;
  isDarkMode: boolean;
  language: Language;
  t: {
    keyboardWarningTitle: string;
    keyboardWarningDesc: string;
    gotIt?: string;
  };
}

export const KeyboardWarningModal: React.FC<KeyboardWarningModalProps> = ({
  isOpen,
  onClose,
  isDarkMode,
  language,
          ...
          >
            ...
            <div className="relative">
              ...
              <div className={`relative w-12 h-12 sm:w-16 sm:h-16 rounded-xl sm:rounded-[24px] flex items-center justify-center shadow-lg ${
                ...
                <Keyboard className="w-6 h-6 sm:w-8 sm:h-8 animate-pulse" />
              </div>
            </div>
