...
import { getFileBase64, b64toBlob, b64toUint8Array, uint8ArrayToB64, parseSecfPayload } from '../utils/fileHelpers';
import { getWasm, b64url_decode } from '../utils/wasmLoader';
import { registerBiometrics, authenticateBiometrics } from '../utils/webAuthn';

interface UseViewSecretProps {
  language: Language;
...
export function useViewSecret({
  ...
  // Check if saved WebAuthn credentials exist for current paste
  useEffect(() => {
    if (viewData?.id) {
      const savedCred = localStorage.getItem(`biometric_cred_${viewData.id}`);
      setHasBiometricsForCurrent(!!savedCred);
    } else {
  ...
  }, [viewData, isSelfDestructed, isDarkMode, triggerShatterExplosion, forceClearClipboard]);
  ...
  const handleBiometricAuthenticate = async () => {
    ...
    if (!credId || !savedPwd) {
      setStatus({ type: 'err', msg: t.biometricAuthFailed });
      return;
    }
    const success = await authenticateBiometrics(credId);
    if (success) {
      setViewPassword(savedPwd);
